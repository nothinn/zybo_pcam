...
int main()
{
	...
	while (1) {
		...
		case 'i':
		{
			xil_printf("Select LED from 0 to 3:\r\n");
			read_char1 = getchar();
			getchar();
			int led_new = read_char1 - '0';


			xil_printf("Write pwm value. Use 4 numbers, such as 0012");
			...
			int pwm_value = 0;
			pwm_value += read_char1 - '0';
			pwm_value *= 10;

			pwm_value += read_char2 - '0';
			pwm_value *= 10;

			pwm_value += read_char4 - '0';
			pwm_value *= 10;

			pwm_value += read_char5 - '0';

			xil_printf("\nPwm: %d", pwm_value);
			xil_printf("\nled: %d", led_new);

			Xil_Out32(LED_DRIVER_BASE_ADDR + led_new * 4, pwm_value);
			break;
		}
